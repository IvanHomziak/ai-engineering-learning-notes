...
Ідея MCP: реалізувати capability один раз як MCP server, після чого різні MCP-compatible hosts можуть підключатися до цього server.
...
```text
Agent orchestration залишається в host/application.
...
- Твердження про популярність MCP, підтримку vendors, registry/discovery, OAuth або ChatGPT support є version-sensitive.
...
MCP також змінює місце виконання tools. У vanilla LangChain-style agent tools часто виконуються всередині application runtime. У MCP flow tool call надсилається до MCP server, і саме server виконує tool.
...
- MCP server володіє tools/resources/prompts і execution;
- external systems залишаються за server-side integration boundary.

Це може покращити reuse, deployment independence, observability, scaling і security governance.
...
Базовий tool calling flow:

```text
user query
-> LLM отримує query + available tools
-> LLM генерує tool call у provider-specific format
-> application parses tool call
-> application executes external tool
...
Якщо tools written directly inside one AI application, вони tightly coupled до цього додатку. MCP дозволяє expose tools through an MCP server, щоб multiple MCP hosts могли reuse the same capability.
...
Source каже, що один MCP client має 1-to-1 connection з одним MCP server. Якщо host підключається до multiple MCP servers, йому потрібні multiple MCP clients.
...
2. Host уже знає available tools з initialization.
3. Host надсилає user query + available tools до LLM.
4. LLM повертає answer або tool call.
5. Якщо є tool call, MCP client надсилає tool name і arguments до MCP server.
...
9. LLM повертає final answer або просить наступний tool call.
...
1. **Tools** — model-controlled functions, які AI може invoke when needed.
2. **Resources** — application-controlled data, static або dynamic.
...
- створити MCP server вручну на Python або Node.js;
- використати AI tools/generators для генерації MCP servers;
- використати community-built open-source MCP servers;
- використати official MCP servers/integrations від companies.

Source advice: не reinvent the wheel. Перед створенням third-party integration MCP server треба перевірити, чи vendor уже має готовий MCP server.
...
- remotely через server-sent events або SSH;
...
Source mentions sampling: MCP server може попросити host AI system згенерувати completion for a prompt. Source каже, що це потужно, але має security і privacy implications.

Source також каже, що application/agent може бути і MCP client, і MCP server, що відкриває composable multi-layer agentic applications.
...
- lower coupling to one LLM vendor або AI app builder;
- tool execution decoupled from agent/application runtime;
- easier debugging, logging, cost tracking і scaling;
...
- malicious server could pretend to be official integration;
...
- LLM selects wrong tool або wrong args.
