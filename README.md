...
## Автоматичне створення секції

Поклади transcript, код, tests і configuration у:

```text
_sources/inbox/<section-id>/
```

Потім у Codex запусти:
...
source analysis
-> atomic manifest
-> detailed lecture draft
-> independent quality review
-> repair when required
-> publication to the mapped root directory
```

Лекція не вважається завершеною лише тому, що Markdown-файли існують. Для
публікації вона повинна пройти незалежний review із результатом
`QUALITY_PASSED`.

Опублікований файл зберігається так:
...
```text
01_LangChain/langchain-section-03/LC-03-01-langchain-core-architecture.md
```

Domain mapping визначено в `config/learning-domain-map.yml`. Відсутня section
folder створюється автоматично через lecture та `index.md` files.
